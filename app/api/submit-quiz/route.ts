...
export async function POST(req: Request) {
  try {
    const { quizId, studentId, answers, studentEmail } = await req.json();

    if (!quizId || !studentId || !answers) {
      return NextResponse.json(
        { error: "Missing quiz ID, student ID, or answers" },
        { status: 400 }
      );
    }
    ...
    let score = 0;
    answers.forEach(
      (studentAnswer: { questionIndex: number; selectedOption: string }) => {
